...
char *hmath_version = "!HVER!HMath: 1.6.0 [SJY 01/06/07]";
...
/* EXPORT->Toeplitz: solve Rx = y where R is Toeplitz, r is input as a vector
   r[1..2*n-1] = r_-(N-1) ...,r_-1, r_0, r_1, r_2, r_3, ..., r_N-1
   Returns false if R is singular */
Boolean Toeplitz(Vector r, Vector x, Vector y, int n)
{
   int j,k,m,m1,m2;
   float pp,pt1,pt2,sd,sgn,sxn;
   float xj,xmax;
   Vector g;
   ...
   for (m=1;m<=n;m++) { /* Main loop over the recursion. */
      ...
      sd = -r[n];
      for (j=1;j<=m;j++) {
         sxn += r[n+m1-j]*x[j]; sd += r[n+m1-j]*g[m-j+1];
      }

      if (sd == 0.0) { FreeVector(&gstack,g); return FALSE; }

      x[m1]=sxn/sd;     /* whence x. */
      for (j=1;j<=m;j++) x[j] -= x[m1]*g[m-j+1];
      if (m1 == n) { FreeVector(&gstack,g); return FALSE; }

      sgn = -r[n-m1]; /* Compute numerator and denominator for G */
      for (j=1;j<=m;j++) sgn += r[n+j-m1]*g[j];

      g[m1]=sgn/sd;    /* whence G */
      k=m; m2=(m+1) >> 1; pp=g[m1];
      for (j=1;j<=m2;j++) {
         pt1=g[j]; pt2=g[k];
         g[j]=pt1-pp*pt2; g[k--]=pt2-pp*pt1;
      }
   } /* Back for another recurrence. */
   HError(1,"Toeplitz:  - should not arrive here!");
   return FALSE;
}
