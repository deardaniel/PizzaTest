...
/* per thread gstack added 24/07/05 (MNS) */

char *hmem_version = "!HVER!HMem: 1.6.0 [SJY 01/06/07]";
...
#define UNLOCK
#else
#define LOCK HMemoryLock();
#define UNLOCK HMemoryUnlock();
#endif
...
void InitMem(void)
{
   ...
   Register(hmem_version);
   CreateHeap(&gstack, "GlobalStack",  MSTAK, 1, 0.0, 100000, ULONG_MAX );
        /* #### should be max size_t */
   CreateHeap(&gcheap, "GlobalCHeap", CHEAP, 1, 0.0, 0,      0 );
...
char *CopyString(MemHeap *x, const char *s)
{
   char *t;
