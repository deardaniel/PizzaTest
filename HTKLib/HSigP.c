...
char *hsigp_version = "!HVER!HSigP: 1.6.0 [SJY 01/06/07]";
...
#define T_CMN  0004     /* trace cepstral mean norm */
...
FBankInfo InitFBank(MemHeap *x, int frameSize, long sampPeriod, int numChans,
    ...
    Boolean doubleFFT,float alpha, float warpLowCut, float warpUpCut)
{
   FBankInfo fb;
   ...
   for (chan=1; chan <= maxChan; chan++) {
      if (alpha == 1.0) {
         ...
         fb.cf[chan] = 1127.0 * log (1.0 +
            WarpFreq(warpLowCut, warpUpCut, cf, minFreq, maxFreq, alpha) /
            700.0);
      }
   }
...
/* EXPORT->FZeroMeanCMN: Zero mean the given data seq with given mean vec */
void FZeroMeanCMN(float *data, int vSize, int n, int step, MeanRec *cmn, Boolean isSpeech)
{
   double sum;
   float *fp,alfa;
   Vector mean;
   ...
   alfa = 1;
   if (isSpeech){
      for (i=0; i<n; i++) alfa = cmn->tConst * alfa;
      for (i=1; i<=vSize; i++){
         sum = 0.0;
         fp = data+i-1;
         for (j=0;j<n;j++){
         ...
         sum /= (double)n;
         cmn->curMeanVec[i] = alfa * (cmn->curMeanVec[i]-sum) + sum;
      }
      cmn->frames += n;
   }

   /* subtract mean from current data block */
   mean = (cmn->frames >= cmn->minFrames)?cmn->curMeanVec:cmn->defMeanVec;
   for (i=1; i<=vSize; i++){
      fp = data+i-1;
      for (j=0;j<n;j++){
         *fp -= mean[i]; fp += step;
   ...
   if (trace&T_CMN && (cmn->frames%10 == 0) && (alfa<1)){
      printf("\nFZeroMean:  cnmFrames=%d minFrames=%d tconst=%.3f",
         cmn->frames,cmn->minFrames,cmn->tConst);
      printf(" alfa=%.3f vSize=%d n=%d step=%d [%s]\n",
         alfa, vSize,n,step,isSpeech?"speech":"sil");
      ShowVector("CMN default",cmn->defMeanVec,20);
      ShowVector("CMN current",cmn->curMeanVec,20);
   }
}
