...
char *atk_release = "1.5";
char *hshell_version = "!HVER!HShell: 1.6.0 [SJY 01/06/07]";
...
static Boolean allowSCPHandling = TRUE;  /* allow SCP handling (ATK option) */
void DisableSCPHandling(void){ allowSCPHandling = FALSE; }
static HTime startTime;                  /* realtime is relative to this start time */
...
/*static char * bundlePath = NULL;*/			/* path of application bundle */
/*#endif*/
...
void InitTimeNow()
{
   struct timeb t;
   ftime(&t);
   startTime = ((float)t.time + (float)t.millitm/1000.0) * 1e7;
}
...
HTime GetTimeNow()
{
   ...
   struct timeb t;
   ftime(&t);
   tnow = ((float)t.time + (float)t.millitm/1000.0) * 1e7;
...
/* EXPORT->Register:module name with HTK version info ver info */
...
Boolean InfoPrinted(void)
{
   ...
   if (printVersionInfo) {
      printf("\nATK/HTK Release %s Version Information\n",atk_release);
      PrVInfo("!HVER!Module:  Version [Who Date]");
...
/* EXPORT WriteConfig: write the config params to a file */
void WriteConfig(FILE *f)
{
   ConfigEntry *e;

   fprintf(f,"  %-14s  %-14s  %16s\n","Module/Tool","Parameter","Value");
   for (e=confList; e!= NULL; e=e->next){
      fprintf(f,"%c %-14s  %-14s  ",(e->param.seen?' ':'#'),
         e->param.user==NULL?"":e->param.user,e->param.name);
      switch(e->param.kind){
         case StrCKind:  fprintf(f,"%16s",e->param.val.s); break;
         case BoolCKind: fprintf(f,"%16s",e->param.val.b?"TRUE":"FALSE"); break;
         case IntCKind:  fprintf(f,"%16d",e->param.val.i); break;
         case FltCKind:  fprintf(f,"%16f",e->param.val.f); break;
      }
      fprintf(f,"\n");
   }
}
...
static ReturnStatus SetScriptFile(char *fn)
{
   ...
   CheckFn(fn);
   if ((script = FOpen(fn,NoFilter,&isPipe)) == NULL){
...
const char *getBundlePath()
{
	if (bundlePath == NULL) {
		...
		CFIndex maxLength = CFStringGetMaximumSizeOfFileSystemRepresentation(cfStringRef);
		
		bundlePath = malloc(sizeof(char) * maxLength);
		CFStringGetFileSystemRepresentation(cfStringRef, bundlePath, maxLength);
...
FILE *FOpen(const char *fname, IOFilter filter, Boolean *isPipe)
{
   ...
   int i;
   Boolean isInput;
   char mode[8],cmd[1028],fbuf[1028];
	/*
#if TARGET_OS_IPHONE
	const char *prefix = getBundlePath();
	char *newFname = malloc(sizeof(char) * strlen(prefix) + strlen(fname) + 2);
	sprintf(newFname, "%s/%s", prefix, fname);
	fname = newFname;
#endif
*/
...
/*#if TARGET_OS_IPHONE
   ...
   *isPipe = FALSE; strcat(mode,"b");
   ...
   for (i=1; i<=maxTry; i++){
      f = fopen(fname,mode);
      if (f==NULL){
			/* may be a perverse DOS file system such as Cygwin */
			if (fname[0] == '/'){
				strcpy(fbuf,fname);
				fbuf[0]=fbuf[1]; fbuf[1]=':';
				f = fopen(fbuf,mode);
			}
      }
      if (f!=NULL) {
...
ReturnStatus InitSource(char *fname, Source *src,  IOFilter filter)
{
   ...
   ok = (src->f = FOpen(fname, filter, &(src->isPipe))) != NULL;
   if (! ok) {
...
/* EXPORT->ReadRawString: get next raw string (i.e. word) from src and
           store it in s ie ignore normal HTK escaping */
...
/* EXPORT->RawReadShort: read n short's from src in ascii or binary */
...
/* EXPORT->RawReadInt: read n ints from src in ascii or binary */
...
/* EXPORT->RawReadFloat: read n floats from src in ascii or binary */
Boolean RawReadFloat(Source *src, float *x, int n, Boolean bin, Boolean swap)
{
   ...
   if (bin){
      ...
      for (j=1; j<=n; j++){
         if (fscanf(src->f,"%e%n",buf,&k) != 1)
	   return FALSE;  /*in windows fscanf can write 1 byte extra
			    int x* */
         *x = buf[0];      /* hence fscanf into local buf then copy (SJY 16/3/06)*/
         x++; count += k;
      }
   }
   src->chcount += count;
...
/* EXPORT->Exit: print config if required and then exit as normal */
...
/* ATK error handling - buffers rather than prints error messages
   to allow application to decide how to report errors.
*/

typedef struct _HRErrorRecord{
  int errcode;
  char message[256];
  struct _HRErrorRecord *next;
}HRErrorRecord;

static HRErrorRecord *hrerrorListHead = NULL;
static HRErrorRecord *hrerrorListTail = NULL;
static HRErrorRecord *hrerrorFreeHead = NULL;
static HRErrorRecord *hrerrorFreeTail = NULL;

/* EXPORT->HRError: print error message on stderr and don't abort*/
void HRError(int errcode, char *message, ...)
{
   ...
   p = (HRErrorRecord *)malloc(sizeof(HRErrorRecord));
   p->next = NULL;
   if (hrerrorListTail==NULL)
     hrerrorListHead = p;
   else
     hrerrorListTail->next = p;
   hrerrorListTail = p;

   p->errcode = errcode;
   va_start(ap,message);
   if (errcode<=0) {
      sprintf(p->message,"WARNING [%+d]  ",errcode);
      vsprintf(buf, message, ap); strcat(p->message,buf);
      sprintf(buf," in %s", arglist[0]); strcat(p->message,buf);
   }else{
      sprintf(p->message,"ERROR [%+d]  ",errcode);
      vsprintf(buf, message, ap); strcat(p->message,buf);
   }
   va_end(ap);
}

/* EXPORT->HRErrorCount: return num messages in buffer */
int HRErrorCount(void)
{
  HRErrorRecord *p;
  int count=0;

  for (p=hrerrorListHead; p!=NULL; p=p->next)
    ++count;
  return count;
}

/* EXPORT->HRErrorGetCode: return the n'th stored code */
int HRErrorGetCode(int n)
{
  HRErrorRecord *p;
  for (p=hrerrorListHead; n>1; p=p->next,--n)
    assert(p != NULL);
  return p->errcode;
}

/* EXPORT->HRErrorGetMess: return the n'th stored message */
...
/* EXPORT->HRErrorReset:  clear the error message buffer */
void HRErrorReset(void)
{
  if (hrerrorFreeHead == NULL)
    hrerrorFreeHead = hrerrorListHead;
  else
    hrerrorFreeTail->next = hrerrorListHead;
  hrerrorFreeTail = hrerrorListTail;
  hrerrorListHead = NULL;
}
...
/* Set and test no console mode */
static Boolean hasConsole = TRUE;
Boolean HasConsole(void) { return hasConsole; }
void SetNoConsoleMode(void) {hasConsole = FALSE; }

/* ATK version of printf, outputs to Monitor when no console */
int ATKprintf(const char * format, ...)
{
   va_list ap;             /* Pointer to unnamed args */
   char buf[1024];
   HThread t = HThreadSelf();
   va_start(ap,format);
#ifdef WIN32
   vsprintf(buf,format,ap);
#else
   vsnprintf(buf,1020, format,ap);
#endif
   if (hasConsole)
		return printf("%s",buf);
	else
		return HPostMessage(t,buf);
}

/* ATK version of fflush, null when no console */
...
ReturnStatus InitShell(int argc, char *argv[], const char *ver)
{
   ...
   arglist[0] = argv[0];
   Register(ver);
   Register(hshell_version);
   Register(hthreads_version);
   ...
   for (i=1; i<argc; i++){
      if (strcmp(argv[i],"-A") == 0){
         ...
         if (strcmp(argv[i],"-C") == 0){
            ...
            if (allowSCPHandling && (strcmp(argv[i],"-S") == 0)){
               if (++i >= argc){
   ...
   if (nParm>0) {
      ...
      if (GetConfBool(cParm,nParm,"EXTENDFILENAMES",&b)) extendedFileNames = b;
		if (GetConfBool(cParm,nParm,"PRINTVERSIONINFO",&b)) {
				printVersionInfo = b;
				if (printVersionInfo) infoPrinted = TRUE;
		}
