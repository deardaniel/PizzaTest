...
/* Basic N-Gram support added 18/11/03 - SJY */
/* Phone tree-structuring added 05/01/04 - SJY */
...
/*
   ...
   In order to accommodate compact arcs we need to know how big they are
...
typedef struct lnode
{
   ...
   Boolean onePred;    /* True if node has a single predecessor */
...
/*
   ...
   bnd is not null, then it is inserted at the start and end of the label
   list.  This is typically used to include a silence model.
*/
...
typedef struct _ShowRec *ShowRecPtr;

typedef LabId WordSet;   /* temp wordset type since no tree structuring yet */
...
struct _NetNode {
   ...
   char    *tag;        /* Semantic tagging information (or logical hmm name) */
   ...
   NetNode *chain;      /* links all net nodes in a single list */
   WordSet wordset;     /* set of word end nodes reachable from this node */
	ShowRecPtr sptr;     /* pointer to show node for visual display */
   Boolean onePred;     /* true if node has one predecessor */
   NetNode *newNetNode; /* used to duplicate nodes in phone structuring */
};

struct _NetLink{
   NetNode *node;       /* Node in network */
   LogFloat linkLM;     /* Transition likelihood */
};
...
/* ------- Lattice read/write support used by HTK ------ */
