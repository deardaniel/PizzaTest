...
char *hadapt_version = "!HVER!HAdapt: 1.6.0 [SJY 01/06/07]";
...
void InitAdapt (XFInfo *xfinfo)
{
   ...
   xfinfo->inXFormFN = NULL;   /* ATK input transform */
   xfinfo->headoc = NULL;
         ...
         for (b = 1; b <= bclass->numClasses; b++) {
            for (i=bclass->ilist[b]; i!=NULL; i=i->next) {
               ...
               if (ai == NULL) {
                  ...
                  nAInfo++;
               } else { /* go the end of the chain and add adaptation info */
...
static void SetAccCache(AdaptXForm *xform)
{
   ...
   ILink i;
   AccCache **ac = NULL;
   int nxflevel = 0, nxfcomb = 1, numXf, nxf, ind;
   int nCache = 0;
   AInfo *ai;
   AdaptXForm *xf;
   XFormSet  *xformSet;
...
static ObsCache *CreateObsCache(XFInfo *xfinfo, int size)
{
   ObsCache  *oc;
...
{
  char path[MAXSTRLEN], fn[MAXSTRLEN];
  /*handle input transform */
   if (xfinfo->useInXForm  && (xfinfo->inXFormFN != NULL)) {

      /* if a transform has been changed the input transform must be
         reapplied */
     PathOf(xfinfo->inXFormFN,path);
     NameOf(xfinfo->inXFormFN,fn);
     AddInXFormDir(hset,path);
     xfinfo->inXForm = LoadOneXForm(hset,fn,NULL);
     SetXForm(xfinfo,hset,xfinfo->inXForm);
