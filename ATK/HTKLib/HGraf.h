...
/*         Thread-safe/Multiple Window version SJY Feb01       */
/*         Linux support added Feb04 MNS                       */
/* ----------------------------------------------------------- */

/* !HVER!HGraf: 1.6.0 [SJY 01/06/07] */

/* HAUDOUT event added - SJY 23/08/05 */

/*
   This module provides a minimal graphics facility.  It provides
   fixed-size windows onto which the graphics and text can
   be painted.  A restricted event handler allows simple interactive
   graphics and window repair.  Supports multi-threading.
...
/*
    WIN32: Each window is resizable and can be minimized to the taskbar,
...
*/
/*
   LINUX: the x11r6 library is used.  Each thread will have it's own
   connection to the X11 server to match the Thread Message queues in
   WIN32.
*/
#ifndef _HGRAF_H_
#define _HGRAF_H_
...
typedef struct _HWindowRec *HWin;     /* abstract window type */
...
enum _HEvent {HMOUSEDOWN,  /* (left) mouse button pressed */
	      ...
	      HTMONUPD,    /* thread monitor update message */
	      HTBUFFER,    /* thread buffer put event */
         HAUDOUT,     /* audio output finished event */
	      HIGNORE,     /* irrelevant message */
	      HWINCLOSE,   /* window close button pressed */
	      HQUIT        /* signal to terminate */
};
enum _KeyType {NORMALKEY, SHIFTKEY, COMMANDKEY, CONTROLKEY,
               ENTERKEY, DELKEY, ESCKEY, LEFTARROWKEY, RIGHTARROWKEY};
...
typedef struct {
   ...
   HWin win;          /* relevant window of rec occurance */
} HEventRec;
...
/*
   Initialise the module.  If terminal only operation required then set
   noGraphics true (required for Linux where no Xserver is available).
*/

HWin MakeHWin(char *wname, int x, int y, int w, int h, int bw);
/*
   Create a window of width w and height h, with top left corner at
   x,y and border width bw, and return a reference to it
*/
...
void TermGraf(void);
/*
   Close down all graphics windows and clean up.  This call is superfluous
...
/*
   If win is not NULL, then next event in event queue relating to win
   is returned.  If an action routine is supplied and there are no
   events pending for win, then that routine is called repeatedly
   until an event occurs.  If win is NULL then all all events are
   returned.
*/

int HEventsPending(HWin win);
/*
   Return the number of events pending in the event queue for win.
   If win is NULL return the number of all pending events.
   WIN32: returns 1 if (number` of events >= 1)
*/

Boolean HMousePos(HWin win, int *x, int *y);
/*
   If pointer is over win, return mouse pos in x, y.
...
/*
   Draw a raised panel of given size and grey level
*/

void HDrawArc(HWin win, int x0, int y0, int x1, int y1,
	      int stAngle, int arcAngle);
void HFillArc(HWin win, int x0, int y0, int x1, int y1,
	      int stAngle, int arcAngle);
/*
...
void HPlotVector(HWin win, int x0,int y0,int x1,int y1, Vector v, int st,
...
/*
   Set the current colour to c in win
*/

void HSetGrey(HWin win, int g);
/*
   Set current colour to grey level g in range 0 to 63 in win
*/

void HSetFontSize(HWin win, int size);
/*
   Set the current font size in points and weight +ve bold, -ve normal,
   0 selects the default font size and weight bold (historical HTK default).
   Actual font size set is closest to that offered by implementation.
*/
...
typedef struct _HButton {
   ...
   int fontSize;              /* font size for label */
   ...
   ButtonId id;
   HWin win;                  /* window owning this button */
   BtnLink next;
   void (*action)(void);      /* ptr to function to call whilst button is down */
} HButton;

HButton *CreateHButton(HWin win, HButton *btnlst, ButtonId btnid, int x,
             int y, int w, int h, char *str, HColour fg,
             HColour bg, int fontSize, void (*action)(void));
/*
   CreateHButton: create a button object at position (x,y) of width/height (w,h),
   foreground/background colours (fg,bg) and str displayed inside in given
   fontsize, (*action) will be executed when the button is de-pressed.
*/

void RedrawHButton(HButton *btn);
/*
    RedrawHButton: redraw the button on the screen.
*/
