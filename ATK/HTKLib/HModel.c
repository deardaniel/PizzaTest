...
char *hmodel_version = "!HVER!HModel: 1.6.0 [SJY 01/06/07]";



/*
fopen -> FOpen for all read only cases to enable cygwin file
system fix to work SJY 03/02/03

FOpen modified to try mapping /f/blah -> f:/blah when read open
fails.  This is needed for Cygwin to work properly with programs
compiled under Windows.  SJY 27/09/03

Macro names synced with HTK. MNS 4/4/05
*/
...
static Boolean synWithLogHMM = TRUE;    /* make ~l macs when synthing CD hmms */
...
static Boolean btreeEnable=FALSE;       /* allow bin decision tree to be loaded */
void EnableBTrees(){btreeEnable=TRUE;}
...
void InitModel(void)
{
  ...
  if (nParm>0){
    ...
    if (GetConfBool(cParm,nParm,"SYNWITHLOGHMM",&b)) synWithLogHMM = b;
...
typedef enum {   /* Only a character big !! */
  ...
  MEAN, VARIANCE, INVCOVAR, XNFORM, GCONST,
...
static struct {
  ...
  { { "BEGINHMM", BEGINHMM }, { "USE", USEMAC },
    ...
    { "INVCOVAR", INVCOVAR }, { "XFORM", XNFORM },
...
static ReturnStatus GetToken(Source *src, Token *tok)
{
  ...
  if (c == '~'){                    /* If macro sym return immediately */
    c = tolower(GetCh(src));
    if (c!='s' && c!='m' && c!='u' && c!='x' && c!='d' && c!='c' && c!='a' &&
	c!='r' && c!='b' && c!='g' && c!='f' && c!='y' && c!='j' && c!='q' &&
...
/* -------------- Binary Decision Tree Handling for Unseen Triphones ------ */

/* Define tree structure - each model has a ~b macro pointing to
   an array [2..N-1] of trees.  Each tree node contains a list of
   patterns.  Each daughter is either another Q or a ptr to a state. */

typedef struct _IPat{	/* list of patterns */
  char *pat;
  struct _IPat *next;
}IPat;
...
typedef struct _TreeNode {
  short  snum;		/* node seq number */
  IPat *quest;		/* Q == pattern list */
  NodeLink yes;	        /* subtree for match with Q */
  NodeLink no;          /* subtree for nomatch with Q */
}TreeNode;

typedef struct  {
  short numStates;     /* Number of states N */
  NodeLink *root;	/* array[2..N-1] of NodeLink */
}TreeMacro;

/* Define question list - only used whilst trees are being constructed */
typedef struct _QEnt *QLink;   /* Linked list of Questions */
typedef struct _QEnt{
  LabId qName;		/* name of question */
  IPat *quest;		/* list of patterns */
  Ptr aux;        /* remembered labid->aux */
  QLink next;
}QEnt;

/* CreateTreeNode: create a new tree node */
static TreeLink CreateTreeNode(MemHeap *mem, int snum)
{
  TreeLink n;

  n = (TreeLink) New(mem,sizeof(TreeNode));
  n->quest = NULL; n->snum = snum;
  n->yes.isLeaf = TRUE; n->yes.link.si = NULL;
  n->no.isLeaf  = TRUE; n->no.link.si = NULL;
  return n;
}

/* CreateTreeMacro: create a new Tree Macro plus top level data structure */
static MLink CreateTreeMacro(HMMSet *hset, LabId bid, int numStates)
{
  TreeMacro *tm;
  int j;

  tm = (TreeMacro *)New(hset->hmem,sizeof(TreeMacro));
  tm->numStates = 0;
  tm->root = (NodeLink *)New(hset->hmem,sizeof(NodeLink)*(numStates-2));
  tm->root -= 2;
  for (j=2; j<numStates; j++) {
    tm->root[j].isLeaf = TRUE; tm->root[j].link.si = NULL;
  }
  return NewMacro(hset,0,'b',bid,tm);
}

/* IsCDModel: true if id is name of a CD model */
static Boolean IsCDModel(LabId id)
{
  if (strchr(id->name,'+') != NULL) return TRUE;
  if (strchr(id->name,'-') != NULL) return TRUE;
  return FALSE;
}
...
static char *ParseAlpha(char *src, char *s)
{
...
static ReturnStatus LoadQuestion(HMMSet *hset, char *qName, char *pattern,
				 MemHeap *qmem, QLink *qHeadPtr, QLink *qTailPtr)
{
  QLink q,c;
  LabId labid;
  IPat *ip;
  ...
  q=(QLink) New(qmem,sizeof(QEnt));
  labid=GetLabId(qName,TRUE);
  for (c=*qHeadPtr;c!=NULL;c=c->next) if (c->qName==labid) break;
  if (c!=NULL){
    HRError(7080,"LoadQuestion: multiple use of quest name %s",qName);
    return(FAIL);
  }
  q->qName=labid;
  q->next = NULL; q->quest = NULL;
  if (*qHeadPtr==NULL) {
    *qHeadPtr = q; *qTailPtr = q;
  } else {
    (*qTailPtr)->next = q; *qTailPtr = q;
  }
  for (p=pattern;*p && isspace(*p);p++);
  if (*p!='{')
    if (p==NULL){
      HRError(7080,"LoadQuestion: no { in itemlist");
      return(FAIL);
    }
  ++p;
  ...
  *r = ',';
  do {                         /* pick up model patterns from item list */
    ...
    while(isspace(*p)) p++;
    if (*p!=','){
      HRError(7080,"LoadQuestion: missing , in itemlist");
      return(FAIL);
    }
    p++;
    ip=(IPat*) New(hset->hmem,sizeof(IPat));
    ip->pat = CopyString(hset->hmem,buf);
    ip->next = q->quest; q->quest = ip;
  } while (p<r);
  q->aux=labid->aux;
  labid->aux=q->quest;
  return(SUCCESS);
...
/* GetLeafNode: get ~s macro structure corresponding to name */
static ReturnStatus GetLeafNode(HMMSet *hset, char *name, NodeLink *n)
{
  MLink m;
  /*   NodeLink n;*/

  m = FindMacroName(hset,'s',GetLabId(name,FALSE));
  if (m == NULL){
    HRError(7080,"GetLeafNode: Macro %s not recognised",name);
    return(FAIL);
  }
  n->isLeaf = TRUE; n->link.si = (StateInfo *)m->structure;
  return(SUCCESS);
}

/* GetNode: find node number n in given tree */
static TreeLink GetNode(TreeLink node, int n)
{
  TreeLink ret;
  ...
  if (node->snum==n) return(node);
    ...
    if (!node->yes.isLeaf) {
      ret=GetNode(node->yes.link.tree,n);
      if (ret != NULL) return(ret);
    }
    if (!node->no.isLeaf) {
      ret=GetNode(node->no.link.tree,n);
      if (ret != NULL) return(ret);
    }
  }
  return(NULL);
...
/* LoadTree: load a single tree from source and link to state info recs */
static ReturnStatus LoadTree(HMMSet *hset, Source *src, NodeLink *tree)
{
  int n;
  char buf[256];
  LabId qname;
  ...
  ReadString(src,buf);
  if (strcmp(buf,"{") != 0){
    if(GetLeafNode(hset,buf,tree)<SUCCESS)return(FAIL);
    else return(SUCCESS);
  }
  tree->isLeaf = FALSE;
  tree->link.tree = CreateTreeNode(hset->hmem,0);
  ReadString(src,buf);
  do {
    sscanf(buf,"%d",&n);
    ReadString(src,buf);	/* read question name */
    qname=GetLabId(buf,FALSE);
    if (qname==NULL?TRUE:qname->aux==0){
      HRError(7080,"LoadTree: Question %s not recognised",buf);
      return(FAIL);
    }
    if((node=GetNode(tree->link.tree,-n))==NULL){
      HRError(7080,"LoadTree: Node %d not in tree",n);
      return(FAIL);
    }
    node->quest = (IPat *)qname->aux;
    ReadString(src,buf);   	/* read no node or state */
    if (sscanf(buf,"%d",&n)==1) {  /* Attach no daughter */
      node->no.isLeaf = FALSE;
      node->no.link.tree = CreateTreeNode(hset->hmem,-n);
    } else
      if(GetLeafNode(hset,buf,&(node->no))<SUCCESS){
	HRError(7080,"LoadTree: GetLeafNode failed",n);
	return(FAIL);
      }
    ReadString(src,buf);   	/* read yes node or state */
    if (sscanf(buf,"%d",&n)==1) {  /* Attach yes daughter */
      node->yes.isLeaf = FALSE;
      node->yes.link.tree = CreateTreeNode(hset->hmem,-n);
    } else
      if(GetLeafNode(hset,buf,&(node->yes))<SUCCESS){
	HRError(7080,"LoadTree: GetLeafNode failed",n);
	return(FAIL);
      }
...
/* UnpackTreeName: unpack "ph[state]" tree label */
static ReturnStatus UnpackTreeName(char *lab, LabId *id, int *state)
{
  char *p,bname[256];

  strcpy(bname,lab);
  if (bname[strlen(bname)-1]==']' && strrchr(bname,'[')!=NULL) {
    p=strchr(bname,'[');
    *p++='\0';
    if (sscanf(p,"%d",state)!=1){
      HRError(7080,"UnpackTreeName: Cannot parse tree state %s %s",lab,p);
      return(FAIL);
    }
  }
  else {
    HRError(7080,"UnpackTreeName: no state indicator in tree name");
    return(FAIL);
  }
  if ((*id=GetLabId(bname,TRUE))==NULL){
    HRError(7080,"UnpackTreeName: Cannot parse tree name %s (base-phone)",lab);
    return(FAIL);
  }
  return(SUCCESS);
}

/* LoadBTrees: load and attach a set of binary trees to give HMM Set */
static ReturnStatus LoadBTrees(Source *inlinedef, HMMSet *hset, char *hmmDir)
{
  Source sb,*src;
  char qname[256],buf[1024],info[256];
  int state,c1,c2,maxState;
  LabId bid;
  MLink m;
  TreeMacro *treeMac;	/* the head of the tree */
  QLink qHead=NULL,qTail=NULL,ql;
  MemHeap qmem;

  CreateHeap(&qmem,"HModel Qmem",MSTAK,1,0.5,1000,10000);
  if (inlinedef!=NULL) {
    if (trace&T_BTR)
      printf("Loading Decision Trees from macro file\n");
    src=inlinedef;
  }
  else {
    if (hmmDir==NULL) strcpy(qname,hset->treeFile);
    else MakeFN(hset->treeFile,hmmDir,NULL,qname);
    if (trace&T_BTR)
      printf("Loading Decision Trees from file %s\n",qname);
    if(InitSource(qname,&sb,NoFilter)<SUCCESS){
      DeleteHeap(&qmem);
      HRError(7010, "LoadBTrees: Can't open file %s", qname);
      return(FAIL);
    }
    src=&sb;
  }
  ...
  while(ReadString(src,info)) {
    ...
    ReadLine(src,buf);
    if(LoadQuestion(hset,qname,buf,&qmem,&qHead,&qTail)<SUCCESS){
      DeleteHeap(&qmem);
      HRError(7080, "LoadBTrees: LoadQuestion failed");
      return(FAIL);
    }
    ++c1;
  }
  ...
  if (c1==0){
    DeleteHeap(&qmem);
    HRError(7080,"LoadBTrees: Questions Expected");
    return(FAIL);
  }
  c1 = c2 = 0;
  maxState = MaxStatesInSet(hset);
  do {
    if (strcmp(info,INLINE_STR)==0) break;
    if(UnpackTreeName(info,&bid,&state)<SUCCESS){
      DeleteHeap(&qmem);
      HRError(7080,"LoadBTrees: UnpackTreeName failed");
      return(FAIL);
    }
    m = FindMacroName(hset,'b',bid);
    if (m==NULL) {
      m = CreateTreeMacro(hset,bid,maxState); ++c1;
    }
    treeMac = (TreeMacro *)m->structure; ++c2;
    if(LoadTree(hset,src,&(treeMac->root[state]))<SUCCESS){
      DeleteHeap(&qmem);
      HRError(7080,"LoadBTrees: LoadTree failed");
      return(FAIL);
    }
    if (treeMac->numStates<=state) treeMac->numStates = state+1;
  } while (ReadString(src,info));
  if (inlinedef==NULL)
    CloseSource(src);
  for (ql=qHead;ql!=NULL;ql=ql->next) ql->qName->aux=ql->aux;
  DeleteHeap(&qmem);
  if (trace&T_BTR)
    printf("  %d tree macros created and %d trees loaded\n",c1,c2);
  return(SUCCESS);
}

/* FindLeafState: filter down given tree until leaf found */
static StateInfo *FindLeafState(char* name, NodeLink n)
{
  TreeLink t;
  IPat *ip;
  Boolean yes = FALSE;

  if (n.isLeaf) return n.link.si;
  t = n.link.tree;
  for (ip=t->quest; ip!=NULL; ip=ip->next)
    if (DoMatch(name,ip->pat)){
      yes = TRUE; break;
    }
  return(FindLeafState(name, (yes)?t->yes:t->no));
}

/* FindMatchingHMM: find physical HMM with matching svec */
static MLink FindMatchingHMM(HMMSet *hset, int N, StateElem *svec)
{
  int h,j;
  MLink m;
  HLink hmm;
  ...
  for (h=0; h<MACHASHSIZE; h++)
	...
	isSame = TRUE;
	for (j=2; j<N; j++)
	  if (svec[j].info != hmm->svec[j].info){
	    isSame = FALSE; break;
	  }
	if (isSame) return m;
      }
...
/* SynthesisePhysHMM: create physical HMM with given svec and name.  Do this
   by finding a physical HMM with same bname and entry state and clone it */
static MLink SynthesisePhysHMM(HMMSet *hset, LabId bid, LabId id, int N,
			       StateElem *svec)
{
  int h,j;
  MLink m;
  ...
  for (h=0; h<MACHASHSIZE && !found; h++)
    for (m=hset->mtab[h]; m!=NULL && !found; m=m->next)
      if (m->type == 'h'){
	strcpy(buf,m->id->name); TriStrip(buf);
	if (bid == GetLabId(buf,FALSE)){
	  hmm = (HLink)m->structure;
	  if (N == hmm->numStates && svec[2].info != hmm->svec[2].info)
	    found = TRUE;
	}
      }
  if (!found){
    HRError(7080,"SynthesisePhysHMM: cant find PhysHMM for %s to clone",id->name);
    return(NULL);
  }
  new = (HLink)New(hset->hmem,sizeof(HMMDef));
  new->owner = hmm->owner;
  new->numStates = hmm->numStates;
  new->svec = (StateElem *)New(hset->hmem,sizeof(StateElem) * new->numStates);
  new->svec -= 2;
  for (j=2; j<new->numStates; j++)
    new->svec[j].info = svec[j].info;
  if ((new->dur=hmm->dur) != NULL){
    if (GetUse(hmm->dur)>0)
      IncUse(hmm->dur);
    else {
      new->dur = CreateSVector(hset->hmem,VectorSize(hmm->dur));
      CopyVector(hmm->dur,new->dur);
    }
  }
  if ((new->transP = hmm->transP) != NULL){
    if (GetUse(hmm->transP)>0)
      IncUse(hmm->transP);
    else {
      new->transP=CreateSMatrix(hset->hmem,NumRows(hmm->transP),NumCols(hmm->transP));
      CopyMatrix(hmm->transP,new->transP);
    }
  }
  new->tIdx=hmm->tIdx;
  new->nUse = 0; new->hook = NULL;
  return NewMacro(hset,0,'h',id,new);
}

/* SynthTriphone: synth given triphone and return ptr to macro.  If
   an existing physical HMM has identical states, then it is used otherwise
   a new physical HMM macro is created.  If synWithLogHMM is set (the default),
   then a new logical HMM macro is created otherwise a pointer to the
   physical HMM macro is returned.  In the latter case, the call to
   FindMacroName will return a pointer to a physical HMM rather than
   a logical HMM.
	*/
static MLink SynthTriphone(HMMSet *hset, LabId id)
{
  char *name=id->name;
  char bname[256];
  MLink m,ms;
  ...
  int j;
  HLink hmm;
  Boolean newPhys = FALSE;

  /* if (!IsCDModel(id)) return NULL;  */
  strcpy(bname,name);
  TriStrip(bname);
  bid = GetLabId(bname,FALSE);
  if ( bid == NULL ||
       (m = FindMacroName(hset,'b',bid)) == NULL ||
       (tm = (TreeMacro *)m->structure)  == NULL  ){
    HRError(7080,"SynthTriphone: no tree macro for %s[%s]",bname,id->name);
    return(NULL);
  }
  svec = (StateElem *)New(&gstack,sizeof(StateElem) * tm->numStates);
  svec -= 2;
  for (j=2; j<tm->numStates; j++)
    svec[j].info = FindLeafState(name,tm->root[j]);
  m = FindMatchingHMM(hset,tm->numStates,svec);
  if (m == NULL) {
    if((m=SynthesisePhysHMM(hset,bid,id,tm->numStates,svec))==NULL){
      HRError(7080,"SynthTriphone: SynthPhysHMM failed");
      return(NULL);
    }
    newPhys = TRUE;
  }
  if (synWithLogHMM) {
    hmm = (HLink)m->structure; hmm->nUse++;
    m = NewMacro(hset,0,'L',id,hmm);  /* Using 'L' stops recursive search */
  }          /* for macro when NewMacro checks that it doesnt alredy exist */
  if (trace&T_BTR){
    printf("%-10s synth'ed from %s hmm [states:",name,(newPhys)?"new":"old");
    for (j=2; j<tm->numStates; j++){
      ms = FindMacroStruct(hset,'s',svec[j].info);
      if (ms==NULL){
	HRError(7080,"SynthTriphone: macro mismatch for %s[%s]",
		bname,name);
	return(NULL);
      }
      printf(" %-9s",ms->id->name);
    }
    printf("]\n");
  }
  Dispose(&gstack,svec+2);
  return m;
}
...
static MixPDF *GetMixPDF(HMMSet *hset, Source *src, Token *tok)
{
   ...
   if (tok->sym==MACRO && tok->macroType=='m') {
      ...
      mp->mIdx = 0;
      if (tok->sym == RCLASS) {
	short r; static Boolean warnedOnce = FALSE;
	if (!ReadShort(src,&r,1,tok->binForm)){
	  HMError(src,"Regression Class Number expected");
	  return(NULL);
	}
	if(!warnedOnce)
	  HMError(src,"Using old model: RCLASS tag no longer supported"), warnedOnce=TRUE;
	if(GetToken(src,tok)<SUCCESS){
	  HMError(src,"GetToken failed");
	  return(NULL);
	}
	 ...
	 return(NULL);
      ...
      } else if (tok->sym==XNFORM || (tok->sym==MACRO && tok->macroType=='x')){
...
void PutTiedWeights(FILE *f, StreamElem *se, Boolean binary)
{
  ...
  for (m=1; m<=M; m++){
    if (v[m] == weight && repCount < 255)
      ...
      if (repCount>0){
	PutTiedWeight(f,(short)(repCount+1),v[m],binary);
	repCount=0;
      } else
  ...
  if (repCount>0){
    PutTiedWeight(f,(short)(repCount+1),-1,binary);
  }else
...
void PutDiscreteWeights(FILE *f, StreamElem *se, Boolean binary)
{
  ...
  for (m=1; m<=M; m++){
    if (v[m] == weight && repCount < 255)
      ...
      if (repCount>0){
	PutMixWeight(f,(short)(repCount+1),v[m],binary);
	repCount=0;
      } else
  ...
  if (repCount>0){
    PutMixWeight(f,(short)(repCount+1),-1,binary);
  }else
		     ...
		     Symbol sym, Boolean inMacro, Boolean binary)
{
  ...
  nUse = GetUse(m);
  if (nUse > 0 || inMacro)
    PutMacroHdr(hset,f,q,(char)(sym==INVCOVAR?'i':'c'),m,binary);
  if (nUse == 0 || inMacro){
...
MLink NewMacro(HMMSet *hset, short fidx, char type, LabId id, Ptr structure)
{
  ...
  if (type == 'L') type = 'l';  /* hack to stop recursive ~l synthesis */
...
/* EXPORT->GetLogHMMMacro: find logical hmm macro def based on given id
	   if not found, then synthesise it */
MLink GetLogHMMMacro(HMMSet *hset, LabId id)
{
  MLink m;

  if (id == NULL)
    HError(7099,"GetLogHMMMacro: null id");
  m = FindMacroName(hset,'l',id);
  if (m==NULL && hset->treeFile != NULL)
    m = SynthTriphone(hset,id);
  return m;
}
...
static ReturnStatus LoadAllMacros(HMMSet *hset, char *fname, short fidx)
{
   ...
   while (tok.sym != EOFSYM){
      ...
      if (type == 'o') {    /* load an options macro */
      ...
      }else if (type == 'q'){    /* get name of bin tree file */
         if (!btreeEnable) {   /* SJY 19/9/02 assumes ~q is always last macro */
            TermScanner(&src);
            return(SUCCESS);
         }
         if (!ReadString(&src,buf)){
            TermScanner(&src);
            HMError(&src,"LoadAllMacros: Tree file name expected");
            return(FAIL);
         }
         if (strcmp(buf,INLINE_STR)==0)
            if(LoadBTrees(&src,hset,NULL)<SUCCESS){
               TermScanner(&src);
               HMError(&src,"LoadAllMacros: LoadBTrees failed");
               return(FAIL);
            }
            hset->treeFile = CopyString(hset->hmem,buf);
            if(GetToken(&src,&tok)<SUCCESS){
               TermScanner(&src);
               HMError(&src,"LoadAllMacros: GetToken failed");
               return(FAIL);
         ...
         if (type == 'h'){    /* load a HMM definition */
            ...
            if( type=='b' && strcmp(buf,INLINE_STR)==0 ) {
               TermScanner(&src);
               HMError(&src,"LoadAllMacros: using old (pre-ATK1.4.1) model sets.  Change ~b macro to ~q");
               return(SUCCESS);
            }
            if(GetToken(&src,&tok)<SUCCESS){
               TermScanner(&src);
...
static ReturnStatus LoadMacroFiles(HMMSet *hset)
{
  ...
  for (mmf=hset->mmfNames; mmf!=NULL; mmf=mmf->next)
    if (!mmf->isLoaded){
      if(LoadAllMacros(hset,mmf->fName,(short)(++i))<SUCCESS) result=FAIL;
      mmf->isLoaded = TRUE;
    }
  return result;
}
...
ReturnStatus LoadHMMSet(HMMSet *hset, char *hmmDir, char *hmmExt)
{
  ...
  if (hset->treeFile!=NULL && strcmp(hset->treeFile,INLINE_STR)!=0)
    if(LoadBTrees(NULL,hset,hmmDir)<SUCCESS){
      HMError(&src,"LoadHMMSet: LoadBTrees failed");
      ResetHMMSet(hset);
      return(FAIL);
    }
...
/* ****SJY_EXTENSION*****  added to support HView */

/* EXPORT->PutMacro: output given macro to file */
void PutMacro(HMMSet *hset, FILE *f, MLink m, Boolean binary)
{
  switch(m->type){
  case 'u':
  ...
  case 'i':
    ...
    /*
	    case 'j':
	    PutBlkTriCovar(hset,f,m,(SBlockTriMat)m->structure,BINVCOVAR,TRUE,binary);
	    break;
	    case 'k':
	    PutSBlockCovar(hset,f,m,(SBlockMat)m->structure,SEMICOVAR,TRUE,binary);
	    break;
    */
  case 'c':
  ...
  case 'h':
    ...
    break;
  default:
    fprintf(f,"Unknown macro type %c\n",m->type);
    break;
  }
}
...
ReturnStatus SaveHMMSet(HMMSet *hset, char *hmmDir, char *hmmExt, Boolean binary)
{
  ...
  for (p=hset->mmfNames,i=1; p!=NULL; p=p->next,i++)
	...
	printf("HModel: saving Macros to %s\n",fname);
      SaveMacros(f,hset,(short)i,binary);
      FClose(f,isPipe);
    }
