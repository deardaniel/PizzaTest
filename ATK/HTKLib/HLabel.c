...
char *hlabel_version = "!HVER!HLabel: 1.6.0 [SJY 01/06/07]";


/* fopen -> FOpen for all read only cases to enable cygwin file
   system fix to work  (ie map of /f/blah -> f:/blah
   SJY 03/02/03
*/
...
static NameCell *NewCell(const char *name)
{
   char *s;
   NameCell *p;
   int len;
   ...
   p->name = s; p->next = NULL; p->lmid = 0; p->aux = NULL;
   return p;
}
...
LabId GetLabId(const char *name, Boolean insert)
{
   int h;
   NameCell *p;
...
void LoadMasterFile(char *fname)
{
   ...
   if ((f = FOpen(fname,NoFilter,&isPipe)) == NULL)
...
Boolean IsMLFFile(char *fn)
{
   ...
   if ((f = FOpen(fn,NoFilter,&isPipe)) == NULL) return FALSE;
   if (fgets(buf,1024,f) == NULL) {
      FClose(f,isPipe); return FALSE;
   }
   if (NoMLFHeader(buf)) {
      FClose(f,isPipe); return FALSE;
   }
   FClose(f,isPipe); return TRUE;
}
      ...
      if ( isMatch ) {
         if (e->type == MLF_IMMEDIATE) {
            ...
            *isMLF=TRUE;
            ...
            f = FOpen(tryspec,NoFilter,&isPipe);
