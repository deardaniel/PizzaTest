...
char *hwave_version = "!HVER!HWave: 1.6.0 [SJY 01/06/07]";
...
static void CheckWrite(void *data, int size, int n, FILE *fp)
{
  if(fwrite(data,size,n,fp)!=n)
    HError(999,"PutWAVHeaderInfo: cannot write to file.\n");
}

/* PutWAVHeaderInfo: write a WAV (WAVEFORM) file header */
static void PutWAVHeaderInfo(FILE *f, Wave w)
{
   char str[128];
   long tmp,rate;
   short val;

   rate = 1e7/w->sampPeriod;
   sprintf(str,"RIFF");      CheckWrite(str,1,4,f);
	tmp=w->nSamples*2+36;     CheckWrite(&tmp,4,1,f);
	sprintf(str,"WAVEfmt ");  CheckWrite(str,1,8,f);
	tmp=16;                   CheckWrite(&tmp,4,1,f);
	val=1;                    CheckWrite(&val,2,1,f);
	val=1;                    CheckWrite(&val,2,1,f);
	tmp=rate;                CheckWrite(&tmp,4,1,f);
	tmp=rate*2;              CheckWrite(&tmp,4,1,f);
	val=2;                    CheckWrite(&val,2,1,f);
	val=16;                   CheckWrite(&val,2,1,f);
	sprintf(str,"data");      CheckWrite(str,1,4,f);
	tmp=w->nSamples*2;           CheckWrite(&tmp,4,1,f);
}
...
ReturnStatus CloseWaveOutput(Wave w, FileFormat fmt, const char *fname)
{
   FILE *f;
   Boolean isPipe;
   char buf[MAXSTRLEN];
   ...
   case WAV:    PutWAVHeaderInfo(f,w);     break;
   ...
   /* If necessary byte swap the waveform data */
   if (fmt==HTK && (!natWriteOrder && vaxOrder)) {
      ByteSwap(w);
   }
   /* Write Body of file */
